...
Updated with gas/platform coin Security helpers.
"""
...
def get_available_gas_lamports(session: Session, wallet_pk: int) -> Result[int]:
    ...
    This is used before trades to ensure we have enough SOL for gas fees.
    """
    try:
        stmt = (
...
def ensure_sufficient_gas(
    ...
    Returns Ok(True) if sufficient, Err with message if not.
    """
...
def spend_gas_security(
    ...
    Similar pattern to record_partial_sell but for gas.
    """
    try:
        sec = session.execute(
...
print("✅ db_manager_v2 loaded — clean modular version ready for notebook (with gas helpers)")